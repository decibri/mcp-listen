...
function captureAudio({ durationMs = 5000, device, outputPath, stopOnSilence = false, silenceMs = 1000, conditioning } = {}) {
  return new Promise((resolve) => {
    ...
    // so name selection is ambiguous by construction and not exposed. The
    // silence-stopped mode adds Silero VAD, which annotates the stream and
    // never gates or alters the samples.
    //
    // Conditioning is spread in first so the mandatory sampleRate/channels
    // (and the device/vad options set below) always win over it. It is an
    // internal capture enhancement, never a tool argument: only voice_query
    // sets it, to improve what whisper transcribes, and the capture_audio
    // tool path passes nothing, so that recording stays byte-identical to the
    // raw device stream, which its byte-exact contract depends on. Every stage
    // voice_query enables is sample-in-sample-out with zero added latency in
    // decibri, so conditioning changes neither the sample count nor the stop
    // timing, and Silero reads decibri's pre-enhancement tap, so the
    // silence-stop is identical whether or not conditioning is set.
    const micOptions = { ...(conditioning || {}), sampleRate: SAMPLE_RATE, channels: CHANNELS };
    if (device !== undefined) {
